function [processed_data features data_label] = preprocess_training_data(data, gesture_time, key_time, sam_freq, label, start)
Ckey = ['C pressed'];
Lkey = ['L pressed'];
Okey = ['O pressed'];
Rkey = ['R pressed'];
...
data_label = [];
%data_label = strings([259,1]);
if start == 1
    data_points1 = length(key_time);
    data_points = data_points1;
elseif start == 2
    data_points2 = length(key_time)-1;
    data_points = data_points2;
end
j=1;
for i=start:2:data_points
